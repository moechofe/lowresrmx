...
class CoreWrapper: NSObject
{
	...
	func compileProgram(sourceCode: String) -> LowResRMXError?
	{
...
class LowResRMXError: NSError, @unchecked Sendable
{
	...
	init(error: CoreError, sourceCode: String)
	{
		...
		super.init(domain: "LowResRMX", code: Int(coreError.code.rawValue), userInfo: [NSLocalizedDescriptionKey: "\(message): \(line)"])
	}
...
func interpreterDidFail(context: UnsafeMutableRawPointer?, coreError: CoreError)
{
...
func diskDriveDidSave(context: UnsafeMutableRawPointer?, diskDataManager: UnsafeMutablePointer<DataManager>?)
{
	let wrapper = Unmanaged<CoreWrapper>.fromOpaque(context!).takeUnretainedValue()
	wrapper.delegate?.coreDiskDriveDidSave(diskDataManager: diskDataManager)
}

func diskDriveIsFull(context: UnsafeMutableRawPointer?, diskDataManager: UnsafeMutablePointer<DataManager>?)
{
	let wrapper = Unmanaged<CoreWrapper>.fromOpaque(context!).takeUnretainedValue()
	wrapper.delegate?.coreDiskDriveIsFull(diskDataManager: diskDataManager)
}

func controlsDidChange(context: UnsafeMutableRawPointer?, controlsInfo: ControlsInfo)
{
	let wrapper = Unmanaged<CoreWrapper>.fromOpaque(context!).takeUnretainedValue()
	wrapper.delegate?.coreControlsDidChange(controlsInfo: controlsInfo)
}

func persistentRamWillAccess(context: UnsafeMutableRawPointer?, destination: UnsafeMutablePointer<UInt8>?, size: Int32)
{
	let wrapper = Unmanaged<CoreWrapper>.fromOpaque(context!).takeUnretainedValue()
	wrapper.delegate?.persistentRamWillAccess(destination: destination, size: size)
}

func persistentRamDidChange(context: UnsafeMutableRawPointer?, data: UnsafeMutablePointer<UInt8>?, size: Int32)
{
