...
{
	@IBOutlet var sourceCodeTextView: EditorTextView!
	@IBOutlet var searchToolbar: SearchToolbar!
	@IBOutlet var indexSideBar: IndexSideBar!
...
//	@IBOutlet var romLabel: UILabel!
	...
	override func viewDidLoad()
	{
		...
		sourceCodeTextView.undoManager?.removeAllActions()
		...
		NotificationCenter.default.addObserver(forName: .EditorSyntaxHighlightingDidChange, object: nil, queue: .main)
		{ [weak self] _ in
			self?.sourceCodeTextView.resetColoration()
			self?.sourceCodeTextView.applyColoration(AppController.shared.editorSyntaxHighlightingMode.rawValue)
		}
		...
		documentStateChangedObserver = NotificationCenter.default.addObserver(forName: UIDocument.stateChangedNotification, object: document, queue: nil)
		{ [weak self] _ in
	...
	func updateEditorInsets()
	{
		...
		if keyboardRect.size.height > 0.0
		{
			if let nav = navigationController
			{
				...
				if keyboardRect.origin.y < textBottomY
				{
	...
	@objc func applicationWillResignActive(_: Notification)
	{
		...
		updateDocument()
		document.autosave(completionHandler: { _ in
			app.endBackgroundTask(bgTask)
			bgTask = UIBackgroundTaskIdentifier.invalid
		})
	}

	@objc func projectManagerDidAddProgram(_: Notification)
	{
		didAddProject = true
	}
	...
	func updateDocument()
	{
		...
		if !state.contains(.closed), sourceCodeTextView.text != document.sourceCode
		{
			...
			document.sourceCode = sourceCodeTextView.text
	...
	{
		guard let versions = NSFileVersion.otherVersionsOfItem(at: document.fileURL)
		else
		{
			assertionFailure()
			return
	...
	func resolveConflict(_ selectedVersion: NSFileVersion?)
	{
		do
		{
			...
			{
				_ = try selectedVersion.replaceItem(at: document.fileURL, options: [])
...
//		}
	...
	@objc func onToolsTapped(_ sender: Any)
	{
		...
		for programUrl in config.programUrls
		{
			...
			alert.addAction(UIAlertAction(title: title, style: .default, handler: { [unowned self] _ in
				editUsingTool(url: programUrl)
			}))
		}

		alert.addAction(UIAlertAction(title: "More...", style: .default, handler: { [unowned self] _ in
			onToolsMoreTapped(sender, config: config)
		}))
	...
	func onToolsMoreTapped(_ sender: Any, config: ToolsMenuConfiguration)
	{
		...
		alert.addAction(UIAlertAction(title: addTitle, style: .default, handler: { [unowned self] _ in
			config.addProgram(url: document.fileURL)
		}))

		alert.addAction(UIAlertAction(title: "Reset Menu", style: .destructive, handler: { _ in
			config.reset()
		}))
		...
		else
		{
			...
			document.autosave(completionHandler: { success in
				...
				else
				{
					...
					//                    activityVC.popoverPresentationController?.backgroundColor=UIColor.cyan
					//                    activityVC.top
	...
	func runProject()
	{
		...
		guard let sourceCode = document.sourceCode
		else
		{
			return
		}
		...
		if let error
		{
			...
			alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: nil))
			alert.addAction(UIAlertAction(title: "Go to Error", style: .default, handler: { [weak self] _ in
	...
	func textViewDidChange(_ textView: UITextView)
	{
		...
		fullHighlightTimer = Timer.scheduledTimer(withTimeInterval: 0.3, repeats: false)
		{ [weak self] _ in
			guard let self else { return }
			sourceCodeTextView.applyColoration(AppController.shared.editorSyntaxHighlightingMode.rawValue)
		}
	...
	func editorTextView(_ editorTextView: EditorTextView, didSelectHelpWith range: NSRange)
	{
		...
		else if results.count > 1
		{
			...
			{
				alert.addAction(UIAlertAction(title: chapter.title, style: .default, handler: { _ in
					AppController.shared.tabBarController.showHelp(chapter: chapter.htmlChapter)
				}))
			}
			alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: nil))
	...
	func searchToolbar(_: SearchToolbar!, didSearch findText: String!, backwards: Bool)
	{
	...
	func searchToolbar(_ toolbar: SearchToolbar!, didReplace findText: String!, with replaceText: String!)
	{
		...
		if sourceText.substring(with: selectedRange).caseInsensitiveCompare(findText) == .orderedSame
		{
			...
			// replace
			if sourceCodeTextView.replaceText(in: selectedRange, with: replaceText)
			{
				sourceCodeTextView.applyColoration(AppController.shared.editorSyntaxHighlightingMode.rawValue, in: NSMakeRange(selectedRange.location, (replaceText as NSString).length))
				sourceCodeTextView.selectedRange = NSMakeRange(selectedRange.location + (replaceText as NSString).length, 0)
				sourceCodeTextView.scrollSelectedRangeToVisible()
			}
		}
