...
class ProjectManager: NSObject
{
	...
	private(set) lazy var localDocumentsUrl: URL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!

	private(set) lazy var applicationSupportUrl: URL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!

	private(set) var ubiquitousContainerUrl: URL?

	var ubiquitousDocumentsUrl: URL?
	{
		ubiquitousContainerUrl?.appendingPathComponent("Documents")
	}
	...
	func importProgram(from url: URL, completion: @escaping ((Error?) -> Void))
	{
		...
		NSFileCoordinator().coordinate(with: [readIntent, writeIntent], queue: queue)
		{ error in
			guard error == nil
			else
			{
	...
	func addProject(originalName: String, programData: Data?, imageData: Data?, completion: @escaping ((Error?) -> Void))
	{
		...
		let programUrl = currentDocumentsUrl.appendingPathComponent(name).appendingPathExtension("rmx")
		let imageUrl = currentDocumentsUrl.appendingPathComponent(name).appendingPathExtension("png")
		...
		NSFileCoordinator().coordinate(with: [programWriteIntent, imageWriteIntent], queue: queue)
		{ error in
			guard error == nil
			else
			{
	...
	private func availableProgramName(original: String) -> String
	{
		...
		var url = currentDocumentsUrl.appendingPathComponent(name).appendingPathExtension("rmx")

		while (try? url.checkPromisedItemIsReachable()) ?? false
		{
			count += 1
			name = "\(original) (\(count))"
			url = currentDocumentsUrl.appendingPathComponent(name).appendingPathExtension("rmx")
		}
	...
	{
		deletePersistentRam(programUrl: item.fileUrl)
	...
	func renameProject(item: ExplorerItem, newName: String, completion: @escaping (Error?) -> Void)
	{
		...
		let programDestUrl = item.fileUrl.deletingLastPathComponent().appendingPathComponent(safeName).appendingPathExtension("rmx")
		...
		NSFileCoordinator().coordinate(with: [programSrcWriteIntent, programDestWriteIntent, imageSrcWriteIntent, imageDestWriteIntent], queue: queue)
		{ error in
			guard error == nil
			else
			{
	...
	func duplicateProject(item: ExplorerItem, completion: @escaping (Error?) -> Void)
	{
		...
		let programDestUrl = currentDocumentsUrl.appendingPathComponent(newName).appendingPathExtension("rmx")
		let imageDestUrl = currentDocumentsUrl.appendingPathComponent(newName).appendingPathExtension("png")
		...
		NSFileCoordinator().coordinate(with: [programReadIntent, programWriteIntent, imageReadIntent, imageWriteIntent], queue: queue)
		{ error in
			guard error == nil
			else
			{
	...
	func saveProjectIcon(programUrl: URL, image: UIImage)
	{
		...
		NSFileCoordinator().coordinate(with: [writeIntent], queue: queue)
		{ error in
	...
	{
		let fileUrl = currentDocumentsUrl.appendingPathComponent("Disk.rmx")
		...
		{ success in
			if success
			{
				...
				{
					completion(nil, NSError(domain: "LowResRMXCoder", code: 0, userInfo: [NSLocalizedDescriptionKey: "“Disk.rmx” is currently not editable."]))
				}
			}
			else
			{
				document.save(to: document.fileURL, for: .forCreating, completionHandler: { success in
					...
					{
						completion(nil, NSError(domain: "LowResRMXCoder", code: 0, userInfo: [NSLocalizedDescriptionKey: "“Disk.rmx” could not be created."]))
					}
				})
			}
		}
			...
			for url in urls
			{
				...
				if shouldCopyBundleProgram(filename: filename)
				{
					...
					else
					{
						...
						NSFileCoordinator().coordinate(writingItemAt: tempUrl, options: [], writingItemAt: targetUrl, options: .forReplacing, error: &coordError)
						{ fromUrl, toUrl in
							do
							{
								print("bundle - replace:", targetUrl.path)
								_ = try FileManager.default.replaceItemAt(toUrl, withItemAt: fromUrl, backupItemName: nil, options: [])
	...
	{
		guard let ubiquitousDocumentsUrl
		else
		{
			assertionFailure()
			return
		...
		do
		{
			let urls = try FileManager.default.contentsOfDirectory(at: localDocumentsUrl, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles])

			for url in urls
			{
				if !url.hasDirectoryPath, url.pathExtension == "rmx"
				{
					var coordError: NSError?
					...
					// program file
					let destUrl = ubiquitousDocumentsUrl.appendingPathComponent(name).appendingPathExtension("rmx")

					NSFileCoordinator().coordinate(writingItemAt: url, options: [], writingItemAt: destUrl, options: .forReplacing, error: &coordError)
					{ fromUrl, toUrl in
						do
						{
					...
					if FileManager.default.fileExists(atPath: imageUrl.path)
					{
						...
						NSFileCoordinator().coordinate(writingItemAt: imageUrl, options: [], writingItemAt: imageDestUrl, options: .forReplacing, error: &coordError)
						{ fromUrl, toUrl in
							do
							{
								print("local - move:", toUrl.path)
