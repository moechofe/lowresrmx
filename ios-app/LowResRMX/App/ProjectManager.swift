        ...
        document.open { (success) in
            if success {
                if document.documentState == .normal {
                    ...
                    completion(nil, NSError(domain: "LowResRMXCoder", code: 0, userInfo: [NSLocalizedDescriptionKey: "“Disk.nx” is currently not editable."]))
                }
            } else {
                document.save(to: document.fileURL, for: .forCreating, completionHandler: { (success) in
                    if success {
                        ...
                        completion(nil, NSError(domain: "LowResRMXCoder", code: 0, userInfo: [NSLocalizedDescriptionKey: "“Disk.nx” could not be created."]))
                    }
                })
            }
        }
