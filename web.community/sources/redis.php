...
class Cluster implements ArrayAccess
{
	...
	function offsetExists(mixed $subkey): bool
	{
	...
	function offsetGet(mixed $subkey): mixed
	{
		assert('is_string($subkey)');
...
class Client
{
	...
	function __call($cmd, array $args)
	{
		...
		while ($bytes_written < $bytes_to_write) {
			if ($bytes_written == 0)
				$rv = fwrite($this->socket, $buffer);
			else
				$rv = fwrite($this->socket, substr($buffer, $bytes_written));
			if ($rv === false || $rv == 0)
				break;
			$bytes_written += $rv;
		}
		...
		if ($state === PHPIREDIS_READER_STATE_COMPLETE)
			...
			throw new ErrorProtocol(phpiredis_reader_get_error($this->reader));
	}
}
